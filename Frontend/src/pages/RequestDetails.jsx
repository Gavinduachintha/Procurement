...
export default function RequestDetails({ user }) {
  ...
  const handleConfirmSpecification = async (action) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to confirm specification')
    } finally {
  ...
  return (
    <div className="request-details">
          ...
          <div className="detail-section">
            ...
            <div className="detail-row">
              ...
              <div className="detail-value">
                <span className="badge" style={{ backgroundColor: getStatusColor(request.status), color: 'white' }}>
                  {request.status}
                </span>
              </div>
            </div>
            ...
            <div className="detail-row">
              ...
              <div className="detail-value">{new Date(request.created_at).toLocaleDateString()}</div>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="detail-section">
            ...
            <div className="detail-row">
              ...
              <div className="detail-value">{new Date(request.required_date).toLocaleDateString()}</div>
            </div>
          </div>
        </Card>
      </div>
      ...
      {request.status === 'SPEC_CHECKED' && request.requested_by === user?.id && (
        <Card className="action-card">
          <div className="detail-section">
            <h2>Specification Review Complete</h2>
            <p>The specification checker has reviewed your specifications. Please confirm to proceed.</p>
