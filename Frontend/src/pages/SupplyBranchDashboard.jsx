...
const buildSummarySheet = (xlsx, entries) => {
  const sheet = xlsx.utils.json_to_sheet(entries);
  sheet["!cols"] = [{ wch: 24 }, { wch: 60 }];
  return sheet;
};

const buildTableSheet = (xlsx, rows, fallbackRow, columnWidths) => {
  const sheet = xlsx.utils.json_to_sheet(rows.length ? rows : [fallbackRow]);
...
const writeWorkbook = (xlsx, workbook, fileName) => {
...
const buildTecWorkbook = (xlsx, { job, rows, generatedBy, generatedAt }) => {
  const workbook = xlsx.utils.book_new();
  const list = Array.isArray(rows) ? rows : [];

  xlsx.utils.book_append_sheet(
    workbook,
    buildSummarySheet(xlsx, [
      {
        Field: "Job Number",
        Value: job?.job_number || job?.jobNumber || job?.id || "-",
      },
      {
        Field: "Procurement Method",
        Value: job?.procurement_method || job?.procurementMethod || "-",
      },
      { Field: "Generated By", Value: generatedBy || "-" },
      { Field: "Generated At", Value: generatedAt || "-" },
    ]),
    sanitizeSheetName("TEC Summary"),
  );
  ...
  xlsx.utils.book_append_sheet(
    ...
    buildTableSheet(
      xlsx,
      list.map((row) => ({
        Supplier: row?.supplierName || row?.supplier_name || "-",
        Item: row?.itemName || row?.item_name || "-",
        Quantity: Number(row?.quantity) || 0,
        "Unit Price": Number(row?.unitPrice ?? row?.unit_price) || 0,
        Decision: row?.decisionStatus || row?.decision_status || "-",
        Recommended: row?.isRecommended || row?.is_recommended ? "Yes" : "No",
        Remarks: row?.remarks || "-",
      })),
      { Message: "No TEC entries available" },
      [24, 24, 12, 14, 16, 12, 40],
    ),
    sanitizeSheetName("TEC Entries"),
  );
...
const buildCommitteeWorkbook = (
  ...
  xlsx.utils.book_append_sheet(
    ...
    buildSummarySheet(xlsx, [
      ...
      { Field: "Committee Type", Value: report?.committee_type || "-" },
      { Field: "Total Amount", Value: Number(report?.total_amount) || 0 },
      { Field: "Generated By", Value: generatedBy || "-" },
      { Field: "Generated At", Value: generatedAt || "-" },
    ]),
    sanitizeSheetName("Committee Summary"),
  );
  ...
  xlsx.utils.book_append_sheet(
    ...
    buildTableSheet(
      ...
      { Message: "No committee summary available" },
      [28, 16, 10, 16],
    ),
    sanitizeSheetName("Supplier Summary"),
  );
  ...
  const itemRows = suppliers.flatMap((supplier) => {
    ...
    return items.map((item) => ({
      ...
      Item: item?.itemName || item?.item_name || "-",
      Description: item?.itemDescription || item?.item_description || "-",
      Quantity: Number(item?.quantity) || 0,
      "Unit Price": Number(item?.unitPrice ?? item?.unit_price) || 0,
      "Item Total": Number(item?.totalAmount ?? item?.total_amount) || 0,
  ...
  xlsx.utils.book_append_sheet(
    ...
    buildTableSheet(
      ...
      [24, 16, 24, 30, 12, 14, 14, 30],
    ),
    sanitizeSheetName("Item Details"),
  );
...
export default function SupplyBranchDashboard({ user }) {
  ...
  const downloadTecReportExcel = async () => {
    if (!selectedJob) {
      return;
    }
    ...
    try {
      ...
      const workbook = buildTecWorkbook({
        xlsx,
        job: selectedJob,
        rows: tecRows,
        generatedBy: user?.full_name || user?.name || user?.email || "User",
      ...
      const fileName = `tec-report-${selectedJob.job_number || selectedJob.id}.xlsx`;
      writeWorkbook(xlsx, workbook, fileName);
      setSuccess("TEC report Excel downloaded.");
    } catch (err) {
      setError(err?.message || "Failed to generate TEC report Excel");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const downloadCommitteeReportExcel = async () => {
    if (!selectedJob || !committeeReport) {
      setError("Generate committee report first");
      return;
    }
    ...
    try {
      const xlsx = await import("xlsx");
      const workbook = buildCommitteeWorkbook({
        xlsx,
        job: selectedJob,
        report: committeeReport,
        generatedBy: user?.full_name || user?.name || user?.email || "User",
      ...
      const fileName = `committee-report-${selectedJob.job_number || selectedJob.id}.xlsx`;
      writeWorkbook(xlsx, workbook, fileName);
      setSuccess("Committee report Excel downloaded.");
    } catch (err) {
      setError(err?.message || "Failed to generate committee report Excel");
    } finally {
      setActionLoading(false);
    }
  };
      ...
      >
        <div className="schedule-screen">
          ...
          <div className="action-buttons">
            ...
            <Button
              variant="secondary"
              onClick={downloadTecReportExcel}
              disabled={actionLoading || tecRows.length === 0}
            >
              Download TEC Excel
            </Button>
            ...
            <Button
              variant="secondary"
              onClick={downloadCommitteeReportExcel}
              disabled={actionLoading || !committeeReport}
            >
              Download Committee Excel
            </Button>
            <Button
              variant="danger"
